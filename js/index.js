...
function isNeighborCellEqualColor(y, x){
    ...
    if(numX - 1>= 0){
        if(newTable[numY][numX - 1].class[0] == newTable[numY][numX].class[0] && !newTable[numY][numX - 1].isCliced){
            newTable[numY][numX].imgNum = null           
           isNeighborCellEqualColor(numY, numX-1)    
        } 
    }
    if(numY - 1>= 0){
        if(newTable[numY - 1][numX].class[0] == newTable[numY][numX].class[0] && !newTable[numY - 1][numX].isCliced){ 
            newTable[numY][numX].imgNum = null           
    ...
    for(let y =0; y < rowValue; y++){        
        for(let x = 0; x < colValue; x++){
           if(newTable[y][x].class == ''){
            ...
            newTable[y][x].class = [(setClassColor(newTable[y][x])), " y" + y + "x" + x]
